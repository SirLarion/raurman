...
use std::{collections::HashMap, env, fs, io, rc::Rc};
    ...
    if !groups.is_empty() {
        for g in groups {
            if let Some(list) = db.json.get(&g) {
                let print_group = PackageDb {
                    json: HashMap::from([(g, list.clone())]),
                };
                println!("{print_group}");
            }
        }
...
pub fn handle_save(pkgs: Vec<Package>, op: &OpType, groups: Vec<Rc<str>>) -> Result<(), AppError> {
    let mut pkgdb = match read_pkgdb() {
        Ok(db) => db,
        Err(AppError::IoError(e)) if e.kind() == io::ErrorKind::NotFound => PackageDb::empty(),
        Err(e) => return Err(e),
    };
